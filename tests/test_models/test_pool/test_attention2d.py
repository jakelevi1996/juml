...
def test_softmaxaverage2d():
    printer = util.Printer("test_softmaxaverage2d", dir_name=OUTPUT_DIR)
    juml.test_utils.set_torch_seed("test_softmaxaverage2d")

    pooler = juml.models.pool.SoftmaxAverage2d()

    x = torch.rand([3, 4, 5, 6])

    pooler.set_shapes(list(x.shape), [11])
    assert repr(pooler) == "SoftmaxAverage2d(num_params=60)"
...
def test_softmaxaverage2d_model():
    printer = util.Printer("test_softmaxaverage2d_model", dir_name=OUTPUT_DIR)
    juml.test_utils.set_torch_seed("test_softmaxaverage2d_model")
